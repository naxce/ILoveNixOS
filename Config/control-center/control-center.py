...
class HotspotPanel(Gtk.Box):
    def __init__(self, go_back):
        ...
        if not ssid:
            self.status_lbl.set_label(
                "Set a name and password, then Save, to enable the toggle above."
            )
    ...
    def _on_toggle(self, state):
        ...
        if state and not ssid:
            self.status_lbl.set_label(
                "Set a name and password, then Save, before enabling."
            )
            GLib.idle_add(
                lambda: (set_switch_active_silently(self.switch, False), False)[1]
            )
            return False
...
class ThemePanel(Gtk.Box):
    ...
    def _populate(self):
        ...
        for theme in ThemeBackend.THEMES:
            ...
            gesture = Gtk.GestureClick()
            gesture.connect("released", lambda *_a, t=theme: self._on_theme_clicked(t))
            row.add_controller(gesture)
            self.list_box.append(row)
